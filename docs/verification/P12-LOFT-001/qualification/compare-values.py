...
With --substitute OLD=NEW (repeatable), every occurrence of the text OLD in
every file is replaced by NEW first: a message a milestone deliberately
changed, in the product and in the tests that quote it, then compares as
before. Each substitution is reported with the number of lines it changed
in each file.
...
def load(path, exclude=None, ignore_line_numbers=False, substitutions=()):
...
def main(argv):
    ...
    for arg in args:
        ...
        if arg == "--ignore-line-numbers":
            ...
            continue
        if arg == "--substitute":
            old, _, new = next(args).partition("=")
            substitutions.append((old, new, {}))
            continue
        paths.append(arg)
    for arg in paths:
        label, _, path = arg.partition("=")
        runs.append((label, load(path, exclude, ignore_line_numbers, substitutions)))
    for old, new, counts in substitutions:
        print(f"substituted '{old}' -> '{new}':")
        for arg in paths:
            label, _, path = arg.partition("=")
            print(f"  {label}: {counts.get(path, 0)} lines")
    ok = True
