@echo off
rem P13 milestone qualification: for each preset, configure, remove every
...
for %%P in (debug release debug-shared) do (
    ...
    rem A failed configure used to fall through to clean, build and ctest and
    rem finish looking complete (found in the P13-COMP-001 adversarial
    rem review). Gate on it the way the clean is already gated.
    if not "!CONFIGURE!"=="0" (
