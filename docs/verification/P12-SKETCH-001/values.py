...
Usage: python values.py <report.xml> <header.txt>

Keeps every passed CHECK_THAT / REQUIRE_THAT and every passed CHECK whose
expansion shows values (not a bare true/false), with the enclosing test case
and section path and any INFO/CAPTURE context. REQUIRE lines (preconditions,
and the 1000-fold repeats of the determinism test) and entries repeated
verbatim within a test case are left out, as are expansions over 1000
characters (whole-file text). Adapted from P11-FEAT-009's values.py.
"""
import sys
...
def walk(node, path, out, infos):
    for child in node:
        if child.tag == "Section":
            ...
            keep = (child.get("success") == "true" and kind.startswith("CHECK")
                    and expanded not in ("true", "false") and len(expanded) <= 1000)
            if keep:
