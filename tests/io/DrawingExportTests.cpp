...
constexpr double kMm = 1e-6; ///< a nanometre on paper; the writers print 4 decimals of a mm
...
std::vector<DxfPair> readDxf(std::string_view text) {
...
std::vector<DxfEntity> dxfEntities(std::string_view text) {
...
std::optional<std::string> dxfHeader(std::string_view text, std::string_view name) {
    const std::vector<DxfPair> pairs = readDxf(text);
...
std::vector<SvgElement> readSvg(std::string_view text) {
...
const SvgElement* findSvg(const std::vector<SvgElement>& elements, std::string_view name) {
    const auto found = std::ranges::find(elements, name, &SvgElement::name);
    return found == elements.end() ? nullptr : &*found;
}

std::vector<const SvgElement*> allSvg(const std::vector<SvgElement>& elements, std::string_view name) {
...
std::string pdfContentStream(std::string_view pdf) {
...
std::vector<double> pdfMediaBox(std::string_view pdf) {
...
std::vector<Point2D> pdfPathPoints(std::string_view content) {
